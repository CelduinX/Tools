...
SCRIPT_NAME="Debian/Ubuntu Docker Bootstrap"
STEP=0
...
require_ubuntu_or_debian() {
  ...
  case "${ID:-}" in
    ...
    debian)
      ...
      ;;
    *)
      die "This script supports Ubuntu and Debian. Detected ID=${ID:-unknown}."
      ;;
  esac

  [[ -n "$DISTRO_CODENAME" ]] || \
    die "Distribution codename not found in $OS_RELEASE_FILE."
...
install_docker_from_official_repo() {
  ...
  local source_line
  source_line="deb [arch=${arch} signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/${DISTRO_ID} ${DISTRO_CODENAME} stable"
